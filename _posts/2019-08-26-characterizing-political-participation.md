...
a key decision variable for voters. Media can be used to deepen the divide between voter profiles to
bolster votes some parties receive.
...
Histograms are colored by the average entropy. Darker red denotes lower average variety in that year's election.
</span>

We can observe that Turkish variety in political thought declined until 2018 elections where October 2015
...
cities is still substantial. This is also associated with the bimodality. Let's investigate it.

<h5 style="margin-bottom:15px">Bimodality in 2018 Elections</h5>
...
having most of the votes in the city. Ankara, the capital city of Turkey, is denoted in yellow star.
</span>

We can see that either AKP (ruling party) or CHP (main opposition party) have the majority in the most
...
bimodality in 2018 elections. Choropleth maps can help us identify if any spatial relationship exist
in this bimodality.

<h5 style="margin-bottom:15px">Bimodality on the Map</h5>
<center><img src="/assets/entropy.png" width="500px"></center>

Turkey map above is colored by the city entropies of 2018 election. Darker green indicates high variety,
yellow indicates average variety and darker red indicates low variety. Map illustrates that the western Turkey
is richer in political variety compared to the rest of the cities in the country. On the other hand,
...
<h5 style="margin-bottom:15px">Factors Influencing the Entropy and Divergence</h5>
<center><img src="/assets/population.png" width="800px"></center>

<span style='font-size:16px; color:gray'>
<b>Figure 3.</b> First row of the figure
indicates entropy versus voter population, median annual disposable income and Gini index where the last two variables are obtained
...
Trends of each subfigure is illustrated with colored lines. Color is based on the majority party in the city.
 Symbol $$\color{gray}r$$  denotes the correlation coefficient between variables, e.g. {% raw %} $$\color{gray}{r_{AKP}}$$ {% endraw %} in subfigure in first row & column
indicate the normalized correlation between entropy and log population. Perfect positive correlation is indicated with a
...
a perfect fit. Overall the figure, correlation values are high in magnitude but goodness of fit is low indicating, there
...
Gini Index is used to measure the income inequality. For better income equality, lower values are desired.
Variety of AKP majority cities increases with higher Gini Index and vice-versa for CHP and HDP. Indicating AKP voting
...
<h5 style="margin-bottom:15px">Conclusions</h5>
...
Analysis on the Turkey shows western coastal regions on Turkey has
...
increases with high voter population, high MADI and greater income inequality. CHP's variety increases with high
voter population, low MADI and with better income equality. HDP's variety increases with lower voter population,
higher MADI and better income equality.

Since AKP is the ruling party and CHP is the main opposition party, AKP and CHP majority cities have low divergences.
On the other hand, HDP majority cities' divergences are substantial. What minimizes the divergence of HDP are lower
voting population, higher MADI and better income equality.
...
<h5 style="margin-bottom:15px">Caveats</h5>
This study is a reductionist one. Politics of Turkey is a complex system where individuals vote based on intricate social decision making. As these results are found by analyzing three decision variables, the reality has much more and there is
no guarantee set on these three being the ones that explain much of the variance.
...
is about a social system, please remember this post does not show how the system works, but a mere proxy for it.
